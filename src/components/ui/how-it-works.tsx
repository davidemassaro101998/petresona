...
const Card = ({
  ...
  return (
    <div
      className={`relative w-full md:w-[280px] transition-transform duration-300 hover:z-30 ${rotate} ${className}`}
    >
      <div className="bg-white dark:bg-neutral-900 p-2 rounded-[25px] shadow-[0px_10px_20px_0px_#D3D3D3] dark:shadow-none border border-neutral-100 dark:border-neutral-800">
        ...
        >
          <span className={`${textColor} text-sm font-semibold uppercase tracking-[0.2em] mb-5`}>
            {number}
          </span>
          <h3 className="text-2xl font-semibold text-primary leading-none mb-[10px]">
            {title}
          </h3>
          <p className="text-muted-foreground text-sm/5 tracking-tight">
            {description}
          </p>
        </div>
      </div>
...
export default function HowItWorks({
  ...
  else if (data.length === 3) height = 420;
