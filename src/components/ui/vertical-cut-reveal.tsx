/**
 * Source: https://21st.dev/@danielpetho/components/vertical-cut-reveal
 ...
 *   - `transition` prop typed as framer-motion's `Transition` instead of the
 *     `AnimationOptions` type (which "motion/react" exposes but
 *     "framer-motion" does not export under that name — same shape, spring
 *     config passed straight through unchanged)
 *   - added a named export (`VerticalCutReveal`) alongside the original
 *     default export, since the brief's usage example imports it by name
 * No other logic, structure, or behavior was changed.
 */
"use client"
...
const VerticalCutRevealImpl = forwardRef<VerticalCutRevealRef, TextProps>(
  (
    {
      children,
      reverse = false,
...
VerticalCutRevealImpl.displayName = "VerticalCutReveal"

export default VerticalCutRevealImpl
export const VerticalCutReveal = VerticalCutRevealImpl
